...
const AffirmationFind = observer(() => {
  ...
  const [affirmations, setAffirmations] = useState(['serntseitnsaetsr','432432432432','serntseitnsaetsr','4432423443','serntseitnsaetsr']);
  ...
  const generateAffirmations = async () => {
    ...
    const start = "give me 6 affirmations that for someone who wants to have the belivef of : "
  ...
  return (
    <div className="h-[100vh] w-screen bg-secondary flex flex-col  px-2">
      ...
      <StandardButton onClick={()=>setIsStart(true)}>Start Affirmations</StandardButton>

        {!isStart && affirmations.length > 0 && affirmations?.map((affirmation, key) => (
        ...
        {isStart && affirmations.length > lineNum &&<ColoredText inputText={affirmations[lineNum]} increaseLineNum={increaseLineNum}/> }
